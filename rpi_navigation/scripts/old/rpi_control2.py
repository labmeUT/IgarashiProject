...
def sendbot(new_cmd):
    ...
    if(new_cmd.linear.x == 0):
        power = int(300 * new_cmd.angular.z)
        if(new_cmd.angular.z > 0):
            rightMotor=format((1460 + power) * 4, 'b').zfill(14)
        ...
        power = int(300 * new_cmd.linear.x)
        if(new_cmd.angular.z > 0):
            rightMotor = format((1460 + power)*4, 'b').zfill(14)
            right_value1=int(rightMotor[7:14],2)
            right_value2=int(rightMotor[0:7],2)

            leftMotor = format((1460 + power * int(1 - math.fabs(new_cmd.angular.z)))*4, 'b').zfill(14)
            left_value1=int(leftMotor[7:14],2)
            left_value2=int(leftMotor[0:7],2)
        else:
            rightMotor = format((1460 + power * int(1 - math.fabs(new_cmd.angular.z)))*4, 'b').zfill(14)
            right_value1=int(rightMotor[7:14],2)
            right_value2=int(rightMotor[0:7],2)
